...
for  i in np.arange(len(station)):
    ...
    # dat2[:half_len] = dat2[:half_len] * hann2[:half_len] # Taper end of anticausal ACFs
...
minkm = 5 # add 2 km to the begining of the line for visibility
...
for i in np.arange(len(dat_fin2)):
    if np.mean(dat_fin2[i]) != 0:
         if (travel_nb3 % 2) == 0: 
             peaks3, _ = find_peaks(dat_fin2[i], height = 0) # Get peaks, multiply by -1 as we want the negative peaks
         else:
         ...
         if len(temppk)==1: # Store the negative peak values if any
             mxpk3[i] = temppk[0]
               ...
               for iu in np.arange(len(temppk)):
                   vg.append(T_P[i]*travel_nb3 - temppk[iu] ) 
               mxpk3[i] = temppk[np.argmin(np.abs(vg))]
                      
        # override automatic selection for a few stations
         if  Line_nb == '4' and (i == 11 or i == 13):
            mxpk3[i] = temppk[-1]
            
         if  Line_nb == '3' and (i == 154): #+7
             mxpk3[i] = temppk[0]   
         elif  Line_nb == '3' and (i == 75):
             mxpk3[i] = temppk[1]
         elif  Line_nb == '3' and (i == 77):
             mxpk3[i] = temppk[1] 
...
if  Line_nb == '1':
    plt.text(-16, -.5, '(b)', fontsize = 18)
    addx = 12
    factval = 4
elif  Line_nb == '2':
     plt.text(-18 , -.5, '(f)', fontsize = 18) 
     addx = 15.5
     factval = 5
elif Line_nb == '3':
...
plt.plot(dist_t, T_P*travel_nb3, color = col, linewidth =3)
plt.plot(dist_t, T_P*travel_nb3+v13, '--', color = col, linewidth = 2)
plt.plot(dist_t, T_P*travel_nb3+v23, '--', color = col, linewidth = 2)
...
if  Line_nb== '1':
    ...
    plt.text(96.4, 2.6, '2p',  weight="bold", color = colt, fontsize = 12)
    plt.text(96.4, 5.1, '$\mathbf{2p^2}$',  weight = "bold", color = 'mediumturquoise', fontsize = 12)
    plt.text(96.4, 7.6, '$\mathbf{2p^3}$', weight = "bold", color = col, fontsize = 12)
elif  Line_nb== '2':
    plt.text(124, 1.5, '$\mathbf{2p^3}$', weight="bold", color = col, fontsize = 12) 
elif  Line_nb== '3':
    plt.text(157, 4, '$\mathbf{2p^3}$', weight="bold", color = col, fontsize = 12)
elif  Line_nb== '4':
    plt.text(109.5, 1.25, '$\mathbf{2p^3}$', weight="bold", color = col, fontsize = 12)
...
if  Line_nb== '1':
    plt.text(-16 ,-.2, '(c)', fontsize = 18)
    plt.text(96.4, 2.5, '2p',  weight="bold", color = colt, fontsize = 12)
elif Line_nb== '2':
    plt.text(-18 ,-.2, '(g)', fontsize = 18)   
    plt.text(124, .5, '2p',  weight="bold", color = colt, fontsize = 12) 
elif Line_nb== '3':
    plt.text(-19 ,-.2, '(c)', fontsize = 18)
    plt.text(157, 1.4, '2p',  weight="bold", color = colt, fontsize = 12)
elif  Line_nb== '4':
    plt.text(-18 ,-.2, '(g)', fontsize = 18)
    plt.text(109.5, .4, '2p',  weight="bold", color = colt, fontsize = 12)
...
plt.plot(dist_t, T_P,color = col, linewidth = 3)

if  Line_nb == '1':
    plt.text(-16 , -.4, '(d)', fontsize = 18)
    plt.legend(loc = 3)
elif  Line_nb == '2':
     plt.text(-18 , -.4, '(h)', fontsize = 18)   
     plt.legend(loc = 4)
elif Line_nb == '3':
